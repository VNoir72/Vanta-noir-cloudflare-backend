...
try{
 ...
 const adminPage=await mf.dispatchFetch('https://api.vantanoir.store/admin',{headers:{...authHeaders,Accept:'text/html'}});assert.equal(adminPage.status,200);const html=await adminPage.text();assert.match(html,/Products/);assert.match(html,/vn-control-sidebar/);assert.match(html,/Store overview/);assert.match(html,/Presence. Power. Precision./);assert.doesNotMatch(html,/signin-with-chatgpt|codex-preview/);
 ...
 const form=new FormData();form.set('file',new File([await readFile(projectPath('public/images/vanta-hero.png'))],'test.png',{type:'image/png'}));
